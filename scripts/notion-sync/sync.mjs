...
const INDEX_FILE = path.join(REPO_ROOT, config.indexFile);
const STATE_FILE = path.join(__dirname, ".generated.json");

// Cusdis comments (https://cusdis.com), configured in config.mjs. With an App
// ID set, a comment box renders at the bottom of every essay; leave it empty
// and no box is rendered (the site is unchanged).
//
...
const CUSDIS_APP_ID = config.cusdis.appId;
const CUSDIS_HOST = config.cusdis.host;
...
function articlePage({ title, description, bodyHtml, tocHtml, slug, ogImage }) {
  const url = `${config.baseUrl}/${slug}.html`;
